#!/usr/bin/env python3
"""
Historical Data Dialog for MeteoApp.
Dialog semplificato per visualizzare i dati storici meteorologici.
"""
...
class HistoricalDataDialog:
    ...
    def update_theme_colors(self):
        ...
        if is_dark:
            return {
                ...
                "text_secondary": "#8b949e", "accent": "#795548", "border": "#30363d"
            }
        else:
            return {
                "bg": "#ffffff", "surface": "#f6f8fa", "text": "#24292f",
                "text_secondary": "#656d76", "accent": "#795548", "border": "#d1d9e0"
            }
    ...
    def show_dialog(self):
        """Show the historical data dialog."""
        if not self.page:
            print("ERROR: Page context not available for HistoricalDataDialog")
            return
    ...
    def create_dialog(self) -> ft.AlertDialog:
        """Create the historical data alert dialog."""
            ...
            content=ft.Column([
                ...
                ft.Column([
                    ft.Row([
                        ...
                        ft.Text("Dati degli ultimi 30 anni", size=14, color=self.colors["text"])
                    ], spacing=8),
                    ft.Row([
                        ft.Icon(ft.Icons.BAR_CHART, color="#FF9800", size=20),
                        ft.Text("Grafici di temperatura e precipitazioni", size=14, color=self.colors["text"])
                    ], spacing=8),
                    ft.Row([
                        ft.Icon(ft.Icons.FILE_DOWNLOAD, color="#4CAF50", size=20),
                        ft.Text("Esportazione dati in CSV/Excel", size=14, color=self.colors["text"])
                    ], spacing=8),
                    ft.Row([
                        ft.Icon(ft.Icons.SEARCH, color="#9C27B0", size=20),
                        ft.Text("Ricerca per periodo specifico", size=14, color=self.colors["text"])
                    ], spacing=8)
                ...
                # Action button
                ft.ElevatedButton(
                    f"🔍 {texts['search_data']}", on_click=lambda _: self.open_data_search(),
        ...
        return ft.AlertDialog(
            ...
            title=ft.Row([
                ft.Icon(ft.Icons.HISTORY, color=self.colors["accent"], size=24),
    ...
    def get_texts(self):
        ...
        if self.language == "en":
            return {
                "title": "Historical Data", "dialog_title": "Historical Data",
                "description": "Access historical weather data for in-depth analysis",
                "search_data": "Search Data", "close": "Close"
            }
        else:  # Italian (default)
            return {
                "title": "Dati Storici", "dialog_title": "Dati Storici",
                "description": "Accedi ai dati meteorologici storici per analisi approfondite",
                "search_data": "Cerca Dati", "close": "Chiudi"
            }
    
    def open_data_search(self):
        """Open historical data search (placeholder)."""
        if self.page:
