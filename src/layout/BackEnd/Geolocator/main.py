...
async def main(page: ft.Page):
    ...
    def create_settings_dialog(handler):
    ...
    async def handle_open_location_settings(e):
        ...
        page.close(create_settings_dialog(handle_open_location_settings))
        page.add(ft.Text(f"open_location_settings: {p}"))

    async def handle_open_app_settings(e):
        p = await gl.open_app_settings_async()
        page.close(create_settings_dialog(handle_open_app_settings))
        page.add(ft.Text(f"open_app_settings: {p}"))

    #location_settings_dialog = create_settings_dialog(handle_open_location_settings)
    #app_settings_dialog = create_settings_dialog(handle_open_app_settings)
        ...
        ft.Row(
            ...
            controls=[
                ...
                ft.OutlinedButton(
                    ...
                    visible=False if page.web else True,
                    on_click=lambda e: page.open(create_settings_dialog(handle_open_location_settings)),
                ),
                ft.OutlinedButton(
                    "Open App Settings",
                    visible=False if page.web else True,
                    on_click=lambda e: page.open(create_settings_dialog(handle_open_app_settings)),
                ),
            ],
        )
    )
